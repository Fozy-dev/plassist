...
_playerok_bot_instance: "PlayerokBot | None" = None


def get_playerok_bot() -> PlayerokBot | None:
    return _playerok_bot_instance


class PlayerokBot:
    def __init__(self):
        global _playerok_bot_instance
        _playerok_bot_instance = self
        self.config = sett.get("config")
        # Загружаем сообщения из Python модуля вместо JSON
        self.messages = load_messages("playerok")
        ...
        self.events_log_path = resolve_runtime_path("bot_data/events_log.json")
        self._events_lock = asyncio.Lock()

    async def _append_event(self, payload: dict):
        ...
        try:
            async with self._events_lock:
                try:
                    with open(self.events_log_path, "r", encoding="utf-8") as file:
                        events = json.load(file)
                except Exception:
                    events = []
                events.append(payload)
                with open(self.events_log_path, "w", encoding="utf-8") as file:
                    json.dump(events[-5000:], file, ensure_ascii=False, indent=4)
        except Exception:
            pass
    ...
    def bump_item(self, item: ItemProfile | MyItem):
        try:
            ...
            logger.error(f"{Fore.LIGHTRED_EX}Ошибка при поднятии предмета «{item.name}»: {Fore.WHITE}{e}")
        ...
        try:
            ...
            if (
                ...
                if new_item.status in (ItemStatuses.PENDING_APPROVAL, ItemStatuses.APPROVED):
                    logger.info(f"{Fore.LIGHTWHITE_EX}«{item_name_frmtd}» {Fore.WHITE}— {Fore.YELLOW}товар восстановлен")
                else:
                    logger.error(f"{Fore.LIGHTRED_EX}Не удалось восстановить предмет «{item_name_frmtd}». Его статус: {Fore.WHITE}{new_item.status.name}")
        except Exception as e:
            logger.error(f"{Fore.LIGHTRED_EX}Ошибка при восстановлении предмета «{item.name}»: {Fore.WHITE}{e}")
    ...
    def request_withdrawal(self) -> bool:
        ...
        try:
            ...
            logger.info(f"{Fore.LIGHTWHITE_EX}{balance}₽ {Fore.WHITE}— {Fore.YELLOW}транзакция на вывод создана")
            return True
        except Exception as e:
    ...
    async def _on_playerok_bot_init(self):
        ...
        def endless_loop():
            while True:
                ...
                if sett.get("auto_complete_deals") != self.auto_complete_deals: self.auto_complete_deals = sett.get("auto_complete_deals")
        ...
        def refresh_account_loop():
            while True:
                ...
                try:
                    ...
                    logger.error(f"{Fore.LIGHTRED_EX}Ошибка при обновлении данных об аккаунте: {Fore.WHITE}{traceback.format_exc()}")
        ...
        def check_banned_loop():
            while True:
                try:
                    ...
                    logger.error(f"{Fore.LIGHTRED_EX}Ошибка при проверки бана на аккаунте: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(900)
            ...
            while True:
                if self.config["playerok"]["auto_restore_items"]["expired"]:
                    try:
                        ...
                        logger.error(f"{Fore.LIGHTRED_EX}Ошибка при авто-восстановлении истёкших предметов: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(45)
            ...
            while True:
                if (
                    ...
                    try:
                        ...
                        logger.error(f"{Fore.LIGHTRED_EX}Ошибка при авто-поднятии предметов: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(3)
            ...
            while True:
                if (
                    ...
                    try:
                        ...
                        logger.error(f"{Fore.LIGHTRED_EX}Ошибка при авто-выводе средств: {Fore.WHITE}{traceback.format_exc()}")
                time.sleep(3)
        ...
        await self._append_event(
            {
                ...
                "username": getattr(event.message.user, "username", ""),
                "text": (event.message.text or "")[:400],
            }
        )
        ...
        await self._append_event(
            {
                ...
                "rating": getattr(event.deal.review, "rating", 0),
            }
        )
        ...
        await self._append_event(
            {
                "type": "problem",
                "deal_id": event.deal.id,
                "item_name": getattr(event.deal.item, "name", ""),
                "username": event.deal.user.username,
            }
        )
    ...
    async def _on_new_deal(self, event: NewDealEvent):
        ...
        await self._append_event(
            {
                "type": "deal",
                "deal_id": event.deal.id,
                "item_name": getattr(event.deal.item, "name", "") or "-",
                "price": getattr(event.deal.item, "price", 0) or 0,
                "username": event.deal.user.username,
            }
        )
        ...
        if self.config["playerok"]["auto_complete_deals"]["enabled"]:
            ...
            deal_name = (event.deal.item.name or "").lower()
            included = any(
                any(
                    phrase.lower() in deal_name or deal_name == phrase.lower()
                    for phrase in included_item
                )
                for included_item in self.auto_complete_deals.get("included", [])
            )
            excluded = any(
                any(
                    phrase.lower() in deal_name or deal_name == phrase.lower()
                ...
                for excluded_item in self.auto_complete_deals.get("excluded", [])
            )

            complete_all = self.config["playerok"]["auto_complete_deals"].get("all", True)
            if (complete_all and not excluded) or (not complete_all and included):
        ...
        await self._append_event(
            {
                "type": "deal_status",
                "deal_id": event.deal.id,
                "status": getattr(event.deal.status, "name", str(event.deal.status)),
                "username": event.deal.user.username,
            }
        )
    ...
    async def run_bot(self):
        ...
        if self.playerok_account.profile.balance:
            logger.info(f" · Баланс: {Fore.LIGHTWHITE_EX}{self.account.profile.balance.value}₽")
            logger.info(f"   · Доступно: {Fore.LIGHTWHITE_EX}{self.account.profile.balance.available}₽")
            logger.info(f"   · В ожидании: {Fore.LIGHTWHITE_EX}{self.account.profile.balance.pending_income}₽")
