...
async def callback_enter_new_included_complete_deal_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        ...
        text=templ.settings_new_complete_included_float_text(
            "🔑 Введите ключевые фразы названия товара для включения в авто-подтверждение."
        ),
...
async def callback_enter_new_excluded_complete_deal_keyphrases(callback: CallbackQuery, state: FSMContext):
    ...
    await throw_float_message(
        ...
        text=templ.settings_new_complete_excluded_float_text(
            "🔑 Введите ключевые фразы названия товара для исключения из авто-подтверждения."
        ),
...
@router.callback_query(F.data == "enter_auto_delivery_goods_page")
async def callback_enter_auto_delivery_goods_page(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    last_page = data.get("last_page", 0)
    await state.set_state(states.AutoDeliveriesStates.waiting_for_page)
    await throw_float_message(
        state=state,
        message=callback.message,
        text=templ.settings_deliv_goods_float_text("📃 Введите номер страницы для перехода:"),
